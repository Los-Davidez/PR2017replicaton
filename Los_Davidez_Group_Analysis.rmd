...
Using the results of two drug therapy studies of 288 cancer cell-lines, this replicathon is based on a 2017 data science study aimed to quantify the agreement between these studies to determine the best chemotherapeutic anticancer treatments. By repeating the analysis made in the 2017 study, we aim to determine its replicability and confirm its results. 
To understand the relationship between viability and drug concentration, we created box plots per dose id (1 being lowest concentration, 8 being highest) in which we could see that higher drug concentrations are consistent with lower viability scores. By comparing the difference in correlations across both studies, we were able to determine that Sorafenib was the drug with the most consistent effect, while Nilotinib was the least consistent. Utilizing summarized data of both studies, we calculated the Pearson and Spearman correlation coefficients for each study’s AUC and IC50 scores. The AUC scores had a greater (Pearson) correlation coefficient (0.66) between studies than the IC50 scores (0.31)-- as well as a better looking scatterplot--, demonstrating more agreement (monotonicity) between them. Since IC50 has a Spearman correlation coefficient (0.55) bigger than its Pearson correlation coefficient (0.31), we can deduce its data is relatively monotonic but non-linear. According to AUC's Spearman correlation coefficient, it also has relatively monotonic data, and we can deduce it is also significantly linear thanks to its Pearson correlation coefficient (0.66). The variance between the correlation coefficients for both studies was highly unusual, showing a lot of inconsistency in its data.
After taking the mean value of the correlation coefficients by drug class, we found that narrow effect drugs demonstrated the highest agreement between studies. However, they do not tend to agree, as the correlation values show weak relationship between studies, the highest value not reaching even 0.5. Cancer cell lines are notorious for their inconsistent drug response between studies; the CCLE and GDSC studies were no different in that regard. In the end, we found that we could confirm the findings of the 2017 reanalysis of the CCLE and GDSC pharmacological studies. Therefore, we understand that it is a highly replicable study, even if our own analysis methods diverged from the original study’s.
```{r}
